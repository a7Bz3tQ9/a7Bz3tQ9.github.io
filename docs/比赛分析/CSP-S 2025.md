...
特殊性质 B：设 `b` 在 $s_{i,1},s_{i,2},t_1,t_2$ 的出现位置分别为 $p_i,q_i,r_1,r_2$。替换前要覆盖原 `b`，则 $pos+p_i-1=r_1$，替换后新 `b` 在 $pos+q_i-1=r_2$。且替换必须完全放得下，$1\leq pos=r_1-p_i+1\leq |t_1|-|s_{i,1}|+1$。可得 $q_i-p_i=r_2-r_1$ 且 $p_i\leq r_1$ 且 $|s_{i,1}|-p_i\leq |t_1|-r_1$。按 $q_i-p_i$ 分组，离线拎出 $r_2-r_1=q_i-p_i$ 的查询，二维数点。
...
??? note "36 分（暴力状压拼上 m=1，再拼上否则输出 0）"
    ...
    signed main(){
        ...
        if(m==1){
            ...
            for(int i=1;i<=n;i++){
                if(a[i]=='0') x++;
                else y++,ans=1ll*ans*(cnt[i-1]-(y-1)+mod)%mod;
            }
